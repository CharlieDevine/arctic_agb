...
glob.fp = 'C:/Users/cjdev/Desktop/ABoVE/Biomass/Santoro_2010_GlobBiomass'
setwd(glob.fp)

#glob = raster('GlobBiomass_2010_ABOVE_nativeres_cropped.tif')
#glob.agg = aggregate(glob, fact = 290, fun = mean, na.rm = TRUE)
...
# ~~~~~~~~~~~~~~~~~~~~~ 2018
# 2017 AGB data
agb.fp = paste(data.fp, 'Biomass', 'Santoro_ESA', '2018', sep = '/')
setwd(agb.fp)

agb.orig.res = raster('ABoVE_ESA_CCI_AGB_2018.tif')
agb.orig.res.croped = crop(agb.orig.res, vod.ext)
agb.above.agg = aggregate(agb.orig.res.croped, fact = 290, fun = mean, na.rm = TRUE)
agb.above.agg.res = resample(agb.above.agg, vod.grid, method = 'ngb')
writeRaster(agb.above.agg.res, 'ABoVE_ESA_CCI_AGB_2018_025deg.tif', format = 'GTiff')
