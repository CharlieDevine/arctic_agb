...
gitrepo.fp = paste(root.fp, 'Github/Above_AGB_VOD', sep = '/')
...
core.domain = readOGR('Core_Region_WGS.shp', stringsAsFactors = FALSE)
entire.domain = readOGR('Extended_Plus_Core_Domains_WGS.shp', stringsAsFactors = FALSE)
...
#wang.mask = mask(wang.area, landmask)
...
# AGB_VM
AGB_VM.pct.change.lc.median = aggregate(AGB_VM.pct.change.lc.df, by = list(AGB_VM.pct.change.lc.df$LC), FUN = function(x){median(x, na.rm = TRUE)})[,1:2]
colnames(AGB_VM.pct.change.lc.median) = c('LC','Median_Pct_Change')
AGB_VM.pct.change.lc.median$Product = 'AGB_VM'

AGB_VM.pct.change.lc.median.stats = agb.lc.pct.change.median.stats(AGB_VM.pct.change.lc.df, 'AGB_VM')

# Wang
wang.pct.change.lc.median = aggregate(wang.pct.change.lc.df, by = list(wang.pct.change.lc.df$LC), FUN = function(x){median(x, na.rm = TRUE)})[,1:2]
colnames(wang.pct.change.lc.median) = c('LC','Median_Pct_Change')
wang.pct.change.lc.median$Product = 'Wang'

wang.pct.change.lc.median.stats = agb.lc.pct.change.median.stats(wang.pct.change.lc.df, 'Wang')

# Xu
xu.pct.change.lc.median = aggregate(xu.pct.change.lc.df, by = list(xu.pct.change.lc.df$LC), FUN = function(x){median(x, na.rm = TRUE)})[,1:2]
colnames(xu.pct.change.lc.median) = c('LC','Median_Pct_Change')
xu.pct.change.lc.median$Product = 'Xu'

xu.pct.change.lc.median.stats = agb.lc.pct.change.median.stats(xu.pct.change.lc.df, 'Xu')

# Liu
liu.pct.change.lc.median = aggregate(liu.pct.change.lc.df, by = list(liu.pct.change.lc.df$LC), FUN = function(x){median(x, na.rm = TRUE)})[,1:2]
colnames(liu.pct.change.lc.median) = c('LC','Median_Pct_Change')
liu.pct.change.lc.median$Product = 'Liu'

liu.pct.change.lc.median.stats = agb.lc.pct.change.median.stats(liu.pct.change.lc.df, 'Liu')

# Combine median pct. change info
agb.median.pct.change.lc = rbind(AGB_VM.pct.change.lc.median,
                                 wang.pct.change.lc.median,
                                 xu.pct.change.lc.median,
                                 liu.pct.change.lc.median)
...
pct.change.boxplot.fun = function(){
  ...
  # Box-whisker plot
  lc.areas.plot = ggplot(data = lc.pct.change.agb[lc.pct],
...
# ------------------ AGB trend (slope) per product and land cover class
AGB_VM.slope.trend.lc.df = agb.lc.mask.fun(AGB_VM.slope.trend[[1]], 'AGB_VM')
colnames(AGB_VM.slope.trend.lc.df) = c('Slope','LC','Product')
wang.slope.trend.lc.df = agb.lc.mask.fun(wang.slope.trend[[1]], 'Wang')
colnames(wang.slope.trend.lc.df) = c('Slope','LC','Product')
xu.slope.trend.lc.df = agb.lc.mask.fun(xu.slope.trend[[1]], 'Xu')
colnames(xu.slope.trend.lc.df) = c('Slope','LC','Product')
liu.slope.trend.lc.df = agb.lc.mask.fun(liu.slope.trend[[1]], 'Liu')
colnames(liu.slope.trend.lc.df) = c('Slope','LC','Product')

AGB_VM.slope.trend.lc.median = aggregate(AGB_VM.slope.trend.lc.df, by = list(AGB_VM.slope.trend.lc.df$LC), FUN = function(x){median(x, na.rm = TRUE)})[,1:2]
colnames(AGB_VM.slope.trend.lc.median) = c('LC','Median_Slope')
AGB_VM.slope.trend.lc.median$Product = 'AGB_VM'

wang.slope.trend.lc.median = aggregate(wang.slope.trend.lc.df, by = list(wang.slope.trend.lc.df$LC), FUN = function(x){median(x, na.rm = TRUE)})[,1:2]
colnames(wang.slope.trend.lc.median) = c('LC','Median_Slope')
wang.slope.trend.lc.median$Product = 'Wang'

xu.slope.trend.lc.median = aggregate(xu.slope.trend.lc.df, by = list(xu.slope.trend.lc.df$LC), FUN = function(x){median(x, na.rm = TRUE)})[,1:2]
colnames(xu.slope.trend.lc.median) = c('LC','Median_Slope')
xu.slope.trend.lc.median$Product = 'Xu'

liu.slope.trend.lc.median = aggregate(liu.slope.trend.lc.df, by = list(liu.slope.trend.lc.df$LC), FUN = function(x){median(x, na.rm = TRUE)})[,1:2]
colnames(liu.slope.trend.lc.median) = c('LC','Median_Slope')
liu.slope.trend.lc.median$Product = 'Liu'

agb.slope.trend.lc = rbind(AGB_VM.slope.trend.lc.median,
                           wang.slope.trend.lc.median,
                           xu.slope.trend.lc.median,
                           liu.slope.trend.lc.median)

agb.slope.trend.lc = reshape(agb.slope.trend.lc, idvar = 'LC', timevar = 'Product', v.names = 'Median_Slope', direction = 'wide')
colnames(agb.slope.trend.lc) = c('Cover','AGB_VM','Wang','Xu','Liu')
agb.slope.trend.lc[,2:5] = round(agb.slope.trend.lc[,2:5],2)

agb.slope.trend.lc.table = flextable(agb.slope.trend.lc) %>%
  align_text_col(align = 'center') %>%
  align_nottext_col(align = 'center') %>%
  width(j = 1, width = 2.5) %>%
  hline(part = 'header') %>%
...
ggsave(filename = paste(figs.fp, 'Annual_SlopeTrend_AGB_Change_per_CoverType.png', sep = '/'),
       gen_grob(agb.slope.trend.lc.table),
       width = 6, height = 2.5)

# Box-whisker figure showing distribution of slope trend 2003-2012 from each AGB product for each LC class
slope.trend.boxplot.fun = function(){
  
  lc.slope.trend.agb = na.omit(rbind(AGB_VM.slope.trend.lc.df,
                                     wang.slope.trend.lc.df,
                                     xu.slope.trend.lc.df,
                                     liu.slope.trend.lc.df))
  
  
  lc.slope.trend.agb$Product = factor(lc.slope.trend.agb$Product, levels = c('AGB_VM','Wang','Xu','Liu'))
  lc.slope.trend.agb$LC = factor(lc.slope.trend.agb$LC, levels = unique(lc.slope.trend.agb$LC))
  
  facet.cols = ggh4x::strip_themed(background_x = ggh4x::elem_list_rect(fill = unname(class.cols)[1:9]))
  
  # Box-whisker plot
  lc.areas.plot = ggplot(data = lc.slope.trend.agb,
                         aes(x = Product, y = Slope, color = Product, fill = Product)) +
    geom_boxplot() +
    stat_summary(fun = median, geom = 'crossbar', color = 'white', fatten = 0.75) +
    ylab('Trend (slope) 2003-2012') +
...
slope.trend.boxplot = slope.trend.boxplot.fun()

ggsave(filename = paste(figs.fp, 'AGB_Pixel_Trends_LC_2003to2012.png', sep = '/'),
       slope.trend.boxplot,
       width = 13, height = 5)