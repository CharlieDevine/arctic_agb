...
setwd(ba.shp.fp)
ba = readOGR('MergedBurnAreas_2006to2010.shp')
...
agb.anoms.ts.fun = function(){
  ...
  agb.fires.plot = ggplot(data = agb.fire.anoms, aes(x = Year, y = AGB, color = Product, linetype = Product, shape = Product)) +
    ...
    geom_hline(yintercept = 0, color = rgb(0,0,0,0.25), size = 1) +
    facet_grid(Fire ~.) +
    ...
    geom_vline(data = data.frame(Fire = 'Pasfield Fire (2010)'), aes(xintercept = pf.year), color = rgb(1,0.5,0,0.6), size = 1.5) +
    ...
    geom_vline(data = data.frame(Fire = 'Ryan Fire (2010)'), aes(xintercept = rf.year), color = rgb(1,0.5,0,0.6), size = 1.5) +
    ...
    geom_rect(data = data.frame(Fire = 'Ryan Fire (2010)'), aes(xmin = 2010, xmax = 2012, ymin = -Inf, ymax = Inf), 
              fill = postfire.col, alpha = 0.25, inherit.aes = FALSE) +
    geom_vline(data = data.frame(Fire = 'Big Creek Fire (2009)'), aes(xintercept = bcf.year), color = rgb(1,0.5,0,0.6), size = 1.5) +
    ...
    geom_rect(data = data.frame(Fire = 'Big Creek Fire (2009)'), aes(xmin = 2009, xmax = 2011, ymin = -Inf, ymax = Inf), 
              fill = postfire.col, alpha = 0.25, inherit.aes = FALSE) +
    geom_vline(data = data.frame(Fire = 'Sid Fire (2008)'), aes(xintercept = sf.year), color = rgb(1,0.5,0,0.6), size = 1.5) +
